  ...
  try {
    ...
    if (!req.file) {
      return res.status(400).send({
        message:
          "Gambar tidak ditemukan, pastikan gambar diunggah dengan benar",
      });
    }
    ...
    if (currentUser.role !== "seller") {
      return res
        .status(403)
        .send({ message: "Hanya seller yang dapat menambahkan produk" });
    }
    ...
    if (!title || !description || !price || !stock) {
      return res
        .status(400)
        .send({ message: "Permintaan tidak valid, pastikan semua data diisi" });
    }
...
const update = async (req, res, _next) => {
  try {
    ...
    if (currentUser.role !== "seller") {
      return res
        .status(403)
        .send({ message: "Hanya seller yang dapat memperbarui produk" });
    }
    ...
    if (!product) {
      return res.status(404).send({
        message:
          "Produk tidak ditemukan atau Anda tidak memiliki izin untuk memperbaruinya",
      });
    }
    ...
    if (!title && !description && !price && !stock && !img_url) {
      return res
        .status(400)
        .send({ message: "Tidak ada data yang diperbarui" });
    }
...
const remove = async (req, res, _next) => {
  try {
    ...
    if (currentUser.role !== "seller") {
      return res
        .status(403)
        .send({ message: "Hanya seller yang dapat menghapus produk" });
    }
    ...
    if (!product) {
      return res.status(404).send({
        message:
          "Produk tidak ditemukan atau Anda tidak memiliki izin untuk menghapusnya",
      });
    }
